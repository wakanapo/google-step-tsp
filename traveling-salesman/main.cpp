...
int main(int argc, char** argv) {
	...
	if (DEBUG)
		cerr << "Reading cities" << endl;
	tsp.readCities(argv[1]);
	if (DEBUG)
		cerr << "Time to read cities: "
				<< ((float) (clock() - t)) / CLOCKS_PER_SEC << " s\n";

	cerr << "number of cities: " << tsp.n << endl;
	...
	if (DEBUG)
		cerr << "Time to fill matrix: " << ((float) (clock() - t2)) / CPS
				<< " s\n";
	...
	tsp.findMST_old();
	if (DEBUG)
		cerr << "Time to find mst: " << ((float) (clock() - t2)) / CPS
				<< " s\n";
	...
	if (DEBUG)
		cerr << "Time to find matching: " << ((float) (clock() - t2)) / CPS
				<< " s\n\n";
	...
	cerr << "time: " << ((float) (end - start)) / CPS << "s\n";
